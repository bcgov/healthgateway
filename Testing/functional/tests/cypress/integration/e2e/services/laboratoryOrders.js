describe("Patient Service", () => {
    ...
    it("Verify Laboratory Unauthorized", () => {
        cy.get("@config").then((config) => {
            ...
            cy.request({
                url: `${config.serviceEndpoints.Laboratory}v1/api/Laboratory/LaboratoryOrders?hdid=${HDID}`,
    ...
    it("Verify Laboratory Expired Delegate Forbidden", () => {
        cy.get("@tokens").then((tokens) => {
            ...
            cy.get("@config").then((config) => {
                ...
                cy.request({
                    url: `${config.serviceEndpoints.Laboratory}v1/api/Laboratory/LaboratoryOrders?hdid=${EXPIREDELEGATEDHDID}`,
                    followRedirect: false,
                    failOnStatusCode: false,
                    auth: {
    ...
    it("Verify Laboratory Delegate Authorized", () => {
        cy.get("@tokens").then((tokens) => {
            ...
            cy.get("@config").then((config) => {
                ...
                cy.request({
                    url: `${config.serviceEndpoints.Laboratory}v1/api/Laboratory/LaboratoryOrders?hdid=${DELEGATEDHDID}`,
                    followRedirect: false,
                    failOnStatusCode: false,
                    auth: {
                        bearer: tokens.access_token,
