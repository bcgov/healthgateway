...
describe("User Profile Notification Settings", () => {
    ...
    function toggleSwitch(channel) {
        const selector = channel === "email" ? sel.email : sel.sms;
    ...
    it("Featuretoggle - profile notification section is not visible", () => {
        ...
        cy.get(sel.header).should("not.exist");
    });
    ...
    it("Featuretoggle - profile notification BC Cancer Screening type is not visible", () => {
        ...
        cy.get(sel.empty).should("be.visible");
    });
    ...
    it("Featuretoggle - profile notification BC Cancer Screening Sms preference is visible but Email preference is not", () => {
        ...
        cy.get(sel.label).should("be.visible");
        cy.get(sel.email).should("not.exist");
        cy.get(sel.sms).should("be.visible");
    });
    ...
    it("Featuretoggle - profile notification BC Cancer Screening Email preference is visible but Sms preference is not", () => {
        ...
        cy.get(sel.sms).should("not.exist");
    });
    ...
    notificationPreferenceScenarios.forEach((scenario) => {
        it(scenario.name, () => {
            ...
            assertSwitch(sel.email, scenario.email);
            assertSwitch(sel.sms, scenario.sms);
        });
    });

    // -------------------------------------------------------
    // Verify note visibility matrix:
    // Shows ONLY when at least one provided channel is unverified.
    // -------------------------------------------------------
    const verifyNoteSel =
        "[data-testid=profile-notification-preferences-verify-note]";

    const verifyNoteScenarios = [
        {
            name: "Hides verify note when email and SMS are verified",
            profile: {
        ...
        {
            name: "Shows verify note when email is unverified but SMS is verified",
            profile: {
                email: "nobody@healthgateway.gov.bc.ca",
                isEmailVerified: false,
                smsNumber: "2506715000",
        ...
        {
            name: "Shows verify note when SMS is unverified but email is verified",
            profile: {
                email: "nobody@healthgateway.gov.bc.ca",
                isEmailVerified: true,
                smsNumber: "2506715000",
        ...
        {
            name: "Shows verify note when email and SMS are unverified",
            profile: {
                email: "nobody@healthgateway.gov.bc.ca",
                isEmailVerified: false,
                smsNumber: "2506715000",
        ...
        {
            name: "Shows verify note when only email exists and is unverified (SMS opted out)",
            profile: {
                email: "nobody@healthgateway.gov.bc.ca",
                isEmailVerified: false,
                smsNumber: undefined,
        ...
        {
            name: "Hides verify note when only email exists and is verified (SMS opted out)",
            profile: {
                email: "nobody@healthgateway.gov.bc.ca",
                isEmailVerified: true,
                smsNumber: undefined,
        ...
        {
            name: "Shows verify note when only SMS exists and is unverified (email opted out)",
            profile: {
                email: undefined,
        ...
        {
            name: "Hides verify note when only SMS exists and is verified (email opted out)",
            profile: {
                email: undefined,
                // isEmailVerified irrelevant when email is undefined
                isEmailVerified: true,
        ...
        {
            name: "Hides verify note when neither email nor SMS exists (both opted out)",
            profile: {
                email: undefined,
                // isEmailVerified irrelevant when email is undefined
                isEmailVerified: true,
    ...
    verifyNoteScenarios.forEach((scenario) => {
        it(scenario.name, () => {
            setupStandardFixtures({
                userProfileBody: buildUserProfileFixture(scenario.profile),
            });
            ...
            if (scenario.shouldShow) {
                cy.get(verifyNoteSel).should("be.visible");
            } else {
                cy.get(verifyNoteSel).should("not.exist");
            }
        });
    });
