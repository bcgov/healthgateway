...
function handleClickNotificationAction(notification: Notification): void {
    ...
    if (entryType) {
        ...
        if (isExternal) {
            ...
            logger.debug(`Internal Route: ${internalRoute}`);
            const resolvedRoute = router.resolve(internalRoute);
            const validInternalRoute = resolvedRoute.route.matched.length > 0;
            logger.debug(`Valid Internal Route: ${validInternalRoute}`);

            if (validInternalRoute) {
                logger.debug(
                    `Current Path: ${route.value.path} - Internal Route: ${internalRoute}`
                );
...
function stripInternalPath(url: string): string {
    ...
    if (match) {
        ...
        logger.debug(`Domain Index: ${domainIndex}`);
