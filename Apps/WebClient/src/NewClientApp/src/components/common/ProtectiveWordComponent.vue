...
const errorMessages = computed(() =>
    protectiveWordAttempts.value > 1 ? ["Invalid protective word. Try again."] : []
);

const isVisible = computed(() => !forceClosed.value && medicationsAreProtected.value && !medicationsAreLoading.value);
...
function fetchMedications(): void {
    medicationStore.retrieveMedications(
        props.hdid,
        ...
        <div class="d-flex justify-center">
            <v-card max-width="700px">
                <v-card-title class="bg-primary px-0">
                    <v-toolbar title="Restricted PharmaNet Records" density="compact" color="primary">
                ...
                <v-card-actions class="d-flex flex-column align-start border-t-sm pa-4">
                    <HgButtonComponent
