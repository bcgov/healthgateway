...
namespace HealthGateway.Medication.Parsers
{
    ...
    public class TILMessageParser : IHNMessageParser<Pharmacy>
    {
        ...
        public HNMessage CreateRequestMessage(string pharmacyId, string userId, string ipAddress)
        {
            ...
            // MSH - Message Header
            m.AddSegmentMSH(
                hnClientConfig.SendingApplication,
                hnClientConfig.SendingFacility,
                hnClientConfig.ReceivingApplication,
                hnClientConfig.ReceivingFacility,
                $"{userId}:{ipAddress}",
                $"{HNClientConfiguration.PATIENT_PROFILE_MESSAGE_TYPE}^00",
                TRACE,
                hnClientConfig.ProcessingID,
                hnClientConfig.MessageVersion);
            ...
            zpl.AddNewField(hnClientConfig.ZPL.TransactionReasonCode); // Transaction Reason Code
            m.AddNewSegment(zpl);
