...
namespace HealthGateway.AccountDataAccess.Patient.Strategy
{
    ...
    /// </summary>
    internal class PatientQueryHdidAll : IPatientQuery
    {
        /// <inheritdoc/>
        public async Task<PatientModel?> GetPatientAsync(PatientRequest request)
        {
            PatientModel? patient = await request.ClientRegistriesDelegate.GetDemographicsAsync(OidType.Hdid, request.Hdid, request.DisabledValidation).ConfigureAwait(true);
