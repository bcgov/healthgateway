...
namespace HealthGateway.AccountDataAccess.Patient.Strategy
{
    ...
    /// </summary>
    internal class PatientPhnEmpiCacheStrategy : IPatientQueryStrategy
    {
        /// <inheritdoc/>
        public async Task<PatientModel?> GetPatientAsync(PatientRequest request)
        {
            ...
            return request.CachedPatient ?? await request.ClientRegistriesDelegate.GetDemographicsAsync(OidType.Phn, request.Identifier, request.DisabledValidation).ConfigureAwait(true);
        }
    }
}
