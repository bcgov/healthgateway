...
namespace HealthGateway.AccountDataAccess.Patient.Strategy
{
    ...
    /// </summary>
    internal class PatientHdidAllCacheStrategy : IPatientQueryStrategy
    {
        /// <inheritdoc/>
        public async Task<PatientModel?> GetPatientAsync(PatientRequest request)
        {
            PatientModel? patient = request.CachedPatient ??
                                    await request.ClientRegistriesDelegate.GetDemographicsAsync(OidType.Hdid, request.Identifier, request.DisabledValidation).ConfigureAwait(true);
