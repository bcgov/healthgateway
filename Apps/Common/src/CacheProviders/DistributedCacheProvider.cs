...
namespace HealthGateway.Common.CacheProviders
{
    ...
    public class DistributedCacheProvider : ICacheProvider
    {
        ...
        {
            return this.cache.Get(key).Deserialize<T?>();
        }

        /// <inheritdoc/>
        public void AddItem<T>(string key, T value, TimeSpan? expiry = null)
        {
            this.cache.Set(key, value.Serialize(), new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = expiry });
        }
        ...
        {
            return (await this.cache.GetAsync(key, ct)).Deserialize<T?>();
        }

        /// <inheritdoc/>
        public async Task AddItemAsync<T>(string key, T value, TimeSpan? expiry = null, CancellationToken ct = default)
        {
            await this.cache.SetAsync(key, value.Serialize(), new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = expiry }, ct);
        }
