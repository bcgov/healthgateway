...
namespace HealthGateway.DrugMaintainer
{
    ...
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ScheduleMapper"/> class.
        ...
        {
            // DRUG_CODE
            this.Map(m => m.DrugProductId).ConvertUsing(row => drugProducts.Where(d => d.DrugCode == row.GetField(0)).First().Id);

            // SCHEDULE
            this.Map(m => m.ScheduleDesc).Index(1);

            // SCHEDULE_FFootnote
            this.Map(m => m.ScheduleDescFrench).Index(2);
        }
    }
}
