...
namespace HealthGateway.DBMaintainer.Apps
{
    ...
    public abstract class BaseDrugApp<T> : IDrugApp
    {
        ...
        protected string ExtractFiles(FileDownload downloadedFile)
        {
            ...
            // Ensure that the extracted folder name is valid
            string extractedFolderName = Path.GetFileNameWithoutExtension(downloadedFile.Name);
            if (string.IsNullOrWhiteSpace(extractedFolderName))
            {
                throw new ArgumentException($"Invalid extracted folder name derived from '{downloadedFile.Name}'");
            }

            string unzippedPath = Path.Combine(downloadedFile.LocalFilePath, extractedFolderName);
            this.Logger.LogInformation("Extracting zip file: {Filename} to {UnzippedPath}", filename, unzippedPath);
