...
namespace HealthGateway.Database.Events
{
    ...
    public class UserProfileEventConsumer
    {
        ...
        protected void OnUserLoggedIn(object? sender, UserLoggedInEventArgs args)
        {
            Task.Run(
                async () =>
                {
                    DbResult<UserProfile> result = await this.userProfileDelegate.UpdateAsync(args.UserProfile);
