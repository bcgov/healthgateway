...
TZ=America/Vancouver npx cypress run \
  --env "bcsc.password=$BCSC_PASSWORD,keycloak.password=$KEYCLOAK_PASSWORD,idir.password=$IDIR_PASSWORD,phoneNumber=$PHONENUMBER,keycloak.phsa.client=$KEYCLOAK_PHSA_CLIENT,keycloak.phsa.secret=$KEYCLOAK_PHSA_SECRET" \
  --record \
  --key $CYPRESS_KEY \
  --parallel \
  --ci-build-id "$buildId" \
