...
echo "Running Cypress Admin Unauthorized Functional Tests"
TZ=America/Vancouver npx cypress run \
    ...
    --ci-build-id "$buildId-Unauthorized" \
    --group "$buildId-Unauthorized" \
    --tag "$tags" \
    --spec "cypress/integration/e2e/**/unauthorized.cy.js" \
    --headless \
    --browser chrome
popd
